...
const JAVASCRIPT_CH02_LESSON_PATH = `${testBasePath()}#/courses/javascript/lessons/javascript-ch02-l04`;
const JAVASCRIPT_CH02_EXERCISE: JavaScriptExerciseLocation = {
...
test.describe('JavaScript Chapter 02 visual regression', () => {
  test.beforeEach(async ({ browserName, page }) => {
    test.skip(browserName !== 'chromium', 'Baseline画像はChromiumで一意に固定する');
    await page.emulateMedia({ reducedMotion: 'reduce' });
    await observeRuntimePage(page);
  ...
  for (const viewport of [
    ...
    test(`javascript-ch02-slide-s04-${viewport.id}`, async ({ page }) => {
      await page.setViewportSize({ width: viewport.width, height: viewport.height });
      await page.goto(`${JAVASCRIPT_CH02_LESSON_PATH}/slides/javascript-ch02-l04-s04`);
      await expect(
        page.getByRole('heading', { level: 1, name: '3を含む条件へ直す' }),
      ).toBeVisible();
      await expect.poll(() => page.evaluate(() => document.fonts.status)).toBe('loaded');
      await page.getByTestId('learning-stage').evaluate((element) => {
        element.scrollTo({ top: 0, left: 0, behavior: 'auto' });
      });
      await expect(page).toHaveScreenshot(`javascript-ch02-slide-s04-${viewport.id}.png`, {
        animations: 'disabled',
        caret: 'hide',
        fullPage: false,
      });
