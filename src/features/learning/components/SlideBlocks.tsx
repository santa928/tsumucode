...
export function SlideBlocks({ blocks, assets, baseUrl }: SlideBlocksProps) {
        ...
        switch (block.type) {
          case 'heading':
            return block.level === 2 ? (
              <h2 key={key} className="text-2xl font-black md:text-3xl">
                {block.text}
              </h2>
            ) : (
              <h3 key={key} className="text-xl font-black md:text-2xl">
                {block.text}
              </h3>
            );
          case 'paragraph':
          ...
          case 'list': {
            ...
            const items = block.items.map((item, itemIndex) => (
              <li key={`${item}-${String(itemIndex)}`} className="pl-1 leading-7">
                {item}
              </li>
            ));
          ...
          case 'code':
            return (
              <div key={key} className="overflow-hidden rounded-workshop-md bg-workshop-ink">
                <p className="border-b border-workshop-muted px-4 py-2 font-mono text-sm font-bold text-workshop-learning">
                  {block.language}
                </p>
                <pre
                  tabIndex={0}
                  ...
                  className="overflow-x-auto p-4 text-workshop-on-primary"
                >
          ...
          case 'practice': {
            ...
            return (
              <section
                ...
                className="rounded-workshop-md border-2 border-workshop-learning bg-workshop-raised p-5 shadow-[var(--tc-shadow-piece)]"
              >
                <h2 id={titleId} className="text-lg font-black">
                  今すぐ試す（約{block.estimatedMinutes}分）
                </h2>
                <p className="mt-3 leading-7">{block.prompt}</p>
                <p className="mt-3 border-l-4 border-workshop-learning pl-3 text-workshop-muted">
          ...
          case 'callout':
            return (
              <div
                ...
                className={`rounded-workshop-md border-l-4 bg-workshop-raised p-5 ${CALLOUT_STYLE[block.tone]}`}
              >
